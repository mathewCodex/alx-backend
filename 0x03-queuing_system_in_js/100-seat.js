...
app.get('/available_seats', (_, res) => {
  ...
  // .then(res => Number.parseInt(result || 0)
...
app.get('/process', (_req, res) => {
   ...
   queue.process('reserve_seat', (_job, done) => {
	...
	.then((availableSeats) => {
	   ...
	   if (availableSeats >= 1) {
	     reservationSeat(availableSeats - 1)
	       .then(() => done());
	   } else {
...
app.listen(PORT, () => {
	...
	.then(() => {
	  ...
	  console.log(`API availble on localhost port ${PORT}`);
	});
});
